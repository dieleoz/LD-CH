# ================================================================
#  GENERAR LAYOUT HTML - COLUMNAS REORGANIZADAS
#  Orden: UFV → PK → Tipo Equipo → Dispositivo → Nombre
# ================================================================

Write-Host ""
Write-Host "Generando Layout HTML (Columnas Reorganizadas)..." -ForegroundColor Cyan
Write-Host ""
...
# HTML con columnas reorganizadas
$htmlContent = @"
<!DOCTYPE html>
<html lang="es">
<head>
    ...
    <div class="container">
        ...
        <div class="table-container">
            <table>
                ...
                <tbody id="tablaBody">
                    <tr><td colspan="10" style="text-align:center; padding:40px; color:#999;">Cargando datos...</td></tr>
                </tbody>
            </table>
        </div>
    ...
    <script>
        ...
        function init() {
            try {
                ...
                console.log('OK: Tabla renderizada con', layoutDataFiltrada.length, 'equipos');
            } catch (error) {
        ...
        function renderizarTabla() {
            ...
            if (layoutDataFiltrada.length === 0) {
                tbody.innerHTML = '<tr><td colspan="10" style="text-align:center; padding:40px; color:#999;">No se encontraron equipos</td></tr>';
                return;
            }
            ...
            const html = layoutDataFiltrada.map(e => {
                ...
                return '<tr class="' + tipoClass + '">' +
                    '<td><strong>' + (e.UFV || '') + '</strong></td>' +
                    '<td><strong>' + (e.Ubicacion_PK || '') + '</strong></td>' +
                    '<td><span style="padding:3px 8px; background:rgba(0,0,0,0.15); border-radius:3px; font-weight:600; font-size:11px;">' + (e.Tipo_Equipo || '') + '</span></td>' +
                    '<td>' + (e.Dispositivo || '') + '</td>' +
                    '<td><strong>' + (e.Nombre_Tablero || '') + '</strong></td>' +
                    '<td>' + (e.Descripcion || '') + '</td>' +
                    '<td style="text-align:center;">' + (e.Ubicacion_Lado || '') + '</td>' +
                    '<td style="text-align:center;">' + (e.Implementacion || '') + '</td>' +
                    '<td style="font-size:11px;">' + (e.Switch_Acceso || '') + '</td>' +
                    '<td style="text-align:center; font-size:11px;">' + (e.Tipo_ITS || '') + '</td>' +
                    '</tr>';
            }).join('');
        ...
        function exportarExcel() {
            ...
            const headers = ['UFV', 'Ubicacion_PK', 'Tipo_Equipo', 'Dispositivo', 'Nombre_Tablero', 'Descripcion', 'Ubicacion_Lado', 'Implementacion', 'Switch_Acceso', 'Tipo_ITS'];
...
Write-Host "  HTML GENERADO - COLUMNAS REORGANIZADAS" -ForegroundColor Green
Write-Host "================================================================" -ForegroundColor Green
Write-Host ""
Write-Host "Orden columnas:" -ForegroundColor Cyan
Write-Host "  1. UFV" -ForegroundColor White
Write-Host "  2. PK" -ForegroundColor White
Write-Host "  3. Tipo Equipo" -ForegroundColor White
Write-Host "  4. Dispositivo" -ForegroundColor White
Write-Host "  5. Nombre" -ForegroundColor White
Write-Host "  + 5 columnas adicionales" -ForegroundColor Gray
Write-Host ""
Write-Host "Archivo: $outputHtml" -ForegroundColor White
