# EXTRACTOR TODA LA COCINA - Completo (Version 2)
...
# Buscar directorios automaticamente
$conceptualPath = $null
$basicaPath = $null

Write-Host "Buscando directorios..." -ForegroundColor Yellow

$directorios = Get-ChildItem -Directory
foreach ($dir in $directorios) {
    if ($dir.Name -match "III.*conceptual") {
        $conceptualPath = $dir.Name
        Write-Host "  Encontrado Conceptual: $($dir.Name)" -ForegroundColor Green
    }
    if ($dir.Name -match "IV.*basica") {
        $basicaPath = $dir.Name
        Write-Host "  Encontrado Basica: $($dir.Name)" -ForegroundColor Green
    }
}
