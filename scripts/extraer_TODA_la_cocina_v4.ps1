# EXTRACTOR TODA LA COCINA - Completo (Version 4)
...
# Obtener nombres exactos de directorios
Write-Host "Obteniendo nombres exactos de directorios..." -ForegroundColor Yellow
...
foreach ($dir in $directorios) {
    if ($dir.Name -match "III.*conceptual") {
        $conceptualPath = $dir.Name
        Write-Host "  Conceptual encontrado: '$($dir.Name)'" -ForegroundColor Green
    }
    if ($dir.Name -match "^IV\.") {
        $basicaPath = $dir.Name
        Write-Host "  Basica encontrado: '$($dir.Name)'" -ForegroundColor Green
    }
}

if (-not $conceptualPath) {
    Write-Host "ERROR: No se encuentra directorio de Ingenieria Conceptual" -ForegroundColor Red
    exit 1
}

if (-not $basicaPath) {
    Write-Host "ERROR: No se encuentra directorio de Ingenieria Basica" -ForegroundColor Red
    exit 1
}
...
try {
    ...
    Write-Host "ERROR analizando Conceptual: $($_.Exception.Message)" -ForegroundColor Red
}
...
try {
    ...
    Write-Host "ERROR analizando Basica: $($_.Exception.Message)" -ForegroundColor Red
}
...
# Mostrar archivos mas grandes
Write-Host ""
Write-Host "ARCHIVOS MAS GRANDES ENCONTRADOS:" -ForegroundColor Yellow

$todosArchivos = @()
foreach ($archivo in $todaLaCocina.ingenieria_conceptual.archivos.GetEnumerator()) {
    $todosArchivos += [PSCustomObject]@{
        Nombre = $archivo.Key
        Tamano = $archivo.Value.tamano_kb
        Categoria = "Conceptual"
        Subcategoria = $archivo.Value.categoria
    }
}
foreach ($archivo in $todaLaCocina.ingenieria_basica.archivos.GetEnumerator()) {
    $todosArchivos += [PSCustomObject]@{
        Nombre = $archivo.Key
        Tamano = $archivo.Value.tamano_kb
        Categoria = "Basica"
...
$todosArchivos | Sort-Object Tamano -Descending | Select-Object -First 10 | ForEach-Object {
    Write-Host "  $($_.Tamano) KB - $($_.Categoria)/$($_.Subcategoria) - $($_.Nombre)" -ForegroundColor White
}
